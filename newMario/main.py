...
spriteHidden = pygame.sprite.Group()
var = 1
coins = 0
levelCoins = 0
...
def game_loop():
    ...
    character1.rect.x = 32
    character1.rect.y = 480

    def check_down_collision():
        nonlocal jumps
        global coins
        if character1.rect.y > utils.display_height - char_height:
            character1.rect.y = utils.display_height - char_height
            jumps = 0
        for sprite in spritelist:
            ...
            if isinstance(sprite, ad.HiddenOff):
                ...
                if isinstance(sprite, ad.Coins):
                    ...
                    character1.rect.y = sprite.rect.y - char_height
                    jumps = 0
                if isinstance(sprite,ad.Spikes):
                    character1.rect.x = 32
                    character1.rect.y = 480
                    game_lost()
                if isinstance(sprite, ad.HiddenOn):
                    if sprite.start == -1:
                        sprite.startTime = time.time()
        ...
        for sprite in spritelist:
            if pygame.sprite.collide_rect(sprite, character1):
                if isinstance(sprite, ad.Coins):
                    ...
                    character1.rect.y = sprite.rect.y + sprite.rect.height
                    jumpcount = 0
                if isinstance(sprite, ad.Destroyable):
                    sprite.kill()
                elif isinstance(sprite, ad.HiddenOff):
                    block = ad.HiddenOn(utils.green, character1.rect.x//utils.tilesize, character1.rect.y//utils.tilesize)
        ...
        for sprite in spritelist:
            if isinstance(sprite, ad.HiddenOff):
                ...
                if isinstance(sprite, ad.Coins):
                    ...
                    character1.rect.x = sprite.rect.x - char_width
        ...
        for sprite in spritelist:
            if isinstance(sprite, ad.HiddenOff):
                ...
                if isinstance(sprite, ad.Coins):
                    ...
                    character1.rect.x = sprite.rect.x + sprite.rect.width
    ...
    def redraw():
        ...
        for sprite in spriteHidden:
            end = time.time()
            if isinstance(sprite, ad.HiddenOn):
                if sprite.startTime != -100 :
                    sprite.start = end - sprite.startTime
                if sprite.start >= 2 and sprite.start <3:
                    sprite.image = pygame.image.load("Game/hidden1.png")
                elif sprite.start >= 3 and sprite.start < 5:
                    sprite.image = pygame.image.load("Game/hidden2.png")
                elif sprite.start >= 5:
                    block = ad.HiddenOff(utils.green, character1.rect.x//utils.tilesize, character1.rect.y//utils.tilesize)
                    block.rect.x = sprite.rect.x
                    block.rect.y = sprite.rect.y
                    sprite.kill()
                    spritelist.add(block)
        ...
        if left:
            gameDisplay.blit(walkleft[walkcount//3], (character1.rect.x,character1.rect.y))
            walkcount += 1
        elif right:
            gameDisplay.blit(walkright[walkcount // 3], (character1.rect.x,character1.rect.y))
            walkcount += 1
        else:
            gameDisplay.blit(character, (character1.rect.x,character1.rect.y))
        coinDrawer(50,50,80,10)
        if button(0,utils.display_height - 50,50,50,utils.gray,utils.lightred, 'kill'):
            character1.rect.x = 32
            character1.rect.y = 480
            game_lost()
        pygame.display.update()
    ...
    while flag:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                quit()
        character1.rect.y += gravity
        check_down_collision()

        key = pygame.key.get_pressed()
        if key[pygame.K_LEFT] and character1.rect.x > vel:
            character1.rect.x -= vel
        ...
        if jumps > 0 and jumpcount >= 0:
            character1.rect.y -= min(abs(jumpcount) * jumpcount,30)
            check_up_collision()
            jumpcount -= 1
        if character1.rect.x >= utils.display_width:
            if coins == levelCoins:
                global var
                global mapData
                var = var + 1
                ...
                character1.rect.x = utils.display_width - 5
